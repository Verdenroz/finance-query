...
class TestDefeatBetaClient:
    ...
    def test_format_financial_data_with_list(self, client):
        ...
        mock_list = [
            {'Revenue': 1000, 'Net Income': 100},
            {'Revenue': 1100, 'Net Income': 110}
        ]
    ...
    def test_format_financial_data_with_nan_values(self, client):
        ...
        mock_df = pd.DataFrame({
            'Revenue': [1000, np.nan, 1200],
            'Net Income': [100, 110, np.nan],
            'Date': pd.to_datetime(['2023-01-01', '2023-04-01', '2023-07-01'])
        })
        ...
        for row_key, row_data in statement_data.items():
            for col_key, value in row_data.items():
                if value is None:
                    # This is expected for NaN values
                    continue
                assert value is not None or pd.isna(value) == False
