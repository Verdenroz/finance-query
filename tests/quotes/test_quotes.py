...
def cached_html_content():
    ...
    """
    cache_dir = Path(__file__).parent / "data" / "quotes"
...
async def test_fetch_quotes(mock_api_response, symbols, bypass_cache):
...
@pytest.mark.parametrize("symbols", [["NVDA"], ["AAPL", "MSFT"]])
async def test_scrape_quotes(cached_html_content, symbols, bypass_cache):
...
@pytest.mark.parametrize("symbols", [["NVDA"], ["AAPL", "MSFT"]])
async def test_fetch_simple_quotes(mock_api_response, symbols, bypass_cache):
...
@pytest.mark.parametrize("symbols", [["NVDA"], ["AAPL", "MSFT"]])
async def test_scrape_simple_quotes(cached_html_content, symbols, bypass_cache):
...
def test_get_quotes_success(test_client, mock_api_response, mock_yahoo_auth, monkeypatch):
...
def test_get_simple_quotes_success(test_client, mock_api_response, mock_yahoo_auth, monkeypatch):
