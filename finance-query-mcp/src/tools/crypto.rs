...
pub async fn get_crypto_coins(count: Option<u32>, vs_currency: Option<String>) -> Result<CallToolResult, McpError> {
    let n = count.unwrap_or(50);
    let currency = vs_currency.as_deref().unwrap_or("usd");
    let coins = crypto::coins(currency, n as usize).await.map_err(finance_err)?;
