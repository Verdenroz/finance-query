...
pub struct RunBacktestParams {
    ...
    // ── Margin / leverage / position sizing (backtesting-depth additions) ──
    /// Maximum notional exposure as a multiple of equity; 1.0 = no leverage (default: 1.0)
    pub max_leverage: Option<f64>,
    /// Maintenance margin ratio that triggers a force-close when leveraged (default: 0.25)
    pub maintenance_margin_pct: Option<f64>,
    /// Annualized borrow rate charged on short positions (default: 0.0)
    pub short_borrow_rate: Option<f64>,
    /// Position sizing scheme: fixed_fraction | atr | volatility_target | fractional_kelly (default: fixed_fraction)
    pub position_sizing: Option<String>,
    /// ATR sizing: fraction of equity to risk per trade (default: 0.01)
    pub sizing_risk_pct: Option<f64>,
    /// ATR sizing: ATR lookback period (default: 14)
    pub sizing_atr_period: Option<u32>,
    /// ATR sizing: stop distance as a multiple of ATR (default: 2.0)
    pub sizing_atr_multiple: Option<f64>,
    /// Volatility-target sizing: target per-bar portfolio volatility (default: 0.01)
    pub sizing_target_vol_pct: Option<f64>,
    /// Volatility-target/fractional-Kelly sizing: trailing lookback window (default: 20)
    pub sizing_lookback: Option<u32>,
    /// Fractional-Kelly sizing: multiplier on the full Kelly fraction (default: 0.5)
    pub sizing_kelly_fraction: Option<f64>,
    /// Fractional-Kelly sizing: trailing closed trades used to estimate win-rate/payoff (default: 20)
    pub sizing_lookback_trades: Option<u32>,
...
impl FinanceTools {
    #[tool(
        description = "Run a backtest of a prebuilt trading strategy (SMA/RSI/MACD/Bollinger/SuperTrend/Donchian) against a symbol's historical data. Returns summary performance metrics, a paginated equity curve, and a paginated trade list. Supports optional leverage, margin, short-borrow cost, and position-sizing schemes (fixed fraction, ATR-based, volatility-target, fractional Kelly)."
    )]
...
mod param_tests {
    ...
    fn run_backtest_params_defaults_optionals_to_none() {
        ...
        assert_eq!(p.position_sizing, None);
    ...
    fn run_backtest_params_preserves_all_fields() {
        let p: RunBacktestParams = serde_json::from_value(json!({
            ...
            "fast_period": 10, "slow_period": 20, "max_leverage": 2.0,
            "maintenance_margin_pct": 0.3, "short_borrow_rate": 0.02,
            "position_sizing": "atr", "sizing_risk_pct": 0.02,
        ...
        assert_eq!(p.short_borrow_rate, Some(0.02));
        assert_eq!(p.position_sizing, Some("atr".to_string()));
        assert_eq!(p.sizing_risk_pct, Some(0.02));
