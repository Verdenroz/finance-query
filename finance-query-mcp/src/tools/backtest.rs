...
use finance_query::backtesting::{
    ...
    PositionSizing, RsiReversal, SmaCrossover, Strategy, SuperTrendFollow,
};
...
const VALID_SIZING_SCHEMES: &str = "fixed_fraction, atr, volatility_target, fractional_kelly";
...
fn build_position_sizing(p: &RunBacktestParams) -> Result<PositionSizing, McpError> {
    match p.position_sizing.as_deref() {
        None | Some("fixed_fraction") => Ok(PositionSizing::FixedFraction),
        Some("atr") => Ok(PositionSizing::Atr {
            risk_pct: p.sizing_risk_pct.unwrap_or(0.01),
            atr_period: p.sizing_atr_period.unwrap_or(14) as usize,
            atr_multiple: p.sizing_atr_multiple.unwrap_or(2.0),
        }),
        Some("volatility_target") => Ok(PositionSizing::VolatilityTarget {
            target_vol_pct: p.sizing_target_vol_pct.unwrap_or(0.01),
            lookback: p.sizing_lookback.unwrap_or(20) as usize,
        }),
        Some("fractional_kelly") => Ok(PositionSizing::FractionalKelly {
            kelly_fraction: p.sizing_kelly_fraction.unwrap_or(0.5),
            lookback_trades: p.sizing_lookback_trades.unwrap_or(20) as usize,
        }),
        Some(other) => Err(invalid_params(format!(
            "Unknown position_sizing '{other}'. Valid schemes: {VALID_SIZING_SCHEMES}"
        ))),
    }
}
...
fn build_config(p: &RunBacktestParams) -> Result<BacktestConfig, McpError> {
    let mut builder = BacktestConfig::builder()
        ...
        .max_leverage(p.max_leverage.unwrap_or(1.0))
        .maintenance_margin_pct(p.maintenance_margin_pct.unwrap_or(0.25))
        .short_borrow_rate(p.short_borrow_rate.unwrap_or(0.0))
        .position_sizing(build_position_sizing(p)?);
...
mod tests {
    ...
    fn base_params(strategy: &str) -> RunBacktestParams {
        RunBacktestParams {
            ...
            sizing_atr_period: None,
            sizing_atr_multiple: None,
            sizing_target_vol_pct: None,
            sizing_lookback: None,
            sizing_kelly_fraction: None,
            sizing_lookback_trades: None,
            equity_limit: None,
            equity_cursor: None,
            trades_limit: None,
            trades_cursor: None,
    ...
    #[test]
    fn build_position_sizing_defaults_to_fixed_fraction() {
        let p = base_params("sma_crossover");
        let sizing = build_position_sizing(&p).unwrap();
        assert!(matches!(sizing, PositionSizing::FixedFraction));
    }

    #[test]
    fn build_position_sizing_builds_atr_scheme() {
        let mut p = base_params("sma_crossover");
        p.position_sizing = Some("atr".to_string());
        p.sizing_risk_pct = Some(0.02);
        p.sizing_atr_period = Some(10);
        p.sizing_atr_multiple = Some(2.5);
        let sizing = build_position_sizing(&p).unwrap();
        match sizing {
            PositionSizing::Atr {
                risk_pct,
                ...
                assert_eq!(risk_pct, 0.02);
                assert_eq!(atr_period, 10);
                assert_eq!(atr_multiple, 2.5);
            }
            _ => panic!("expected Atr sizing"),
        }
    }

    #[test]
    fn build_position_sizing_rejects_unknown_scheme() {
        let mut p = base_params("sma_crossover");
        p.position_sizing = Some("bogus".to_string());
        assert!(build_position_sizing(&p).is_err());
    }
    ...
    #[test]
    fn build_config_applies_leverage_and_sizing_overrides() {
        let mut p = base_params("sma_crossover");
