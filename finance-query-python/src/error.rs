...
pub fn to_py_err(err: FinanceError) -> PyErr {
    ...
    match err {
        ...
        | FinanceError::NoProviderAvailable { .. } => ConfigError::new_err(msg),
        ...
        // Feature-gated variants on the upstream crate (e.g. `TranslationError`
        // behind its `translation` feature) and any future additions fall back
        // to the base type. The python crate cannot `cfg` on finance-query's
        // own features, so a catch-all keeps the bindings buildable under every
        // upstream feature combination (e.g. workspace feature unification).
        _ => FinanceQueryError::new_err(msg),
    }
}
