...
export interface GqlBacktestMetrics {
...
export interface GqlBacktestParams {
  allowShort?: boolean;
  commissionPct?: number;
  exitAtMiddle?: boolean;
  fastPeriod?: number;
...
export interface GqlBacktestResult {
  diagnostics?: string[];
  endTimestamp: number;
  /** Portfolio value per bar, oldest first. */
  equityCurve: Page_GqlEquityPoint;
  finalEquity: number;
  initialCapital: number;
...
export interface GqlEquityPoint {
  drawdownPct: number;
  equity: number;
  timestamp: number;
}
...
export interface GqlTrade {
  ...
  entrySignal: GqlTradeSignal;
  entryTimestamp: number;
  exitPrice: number;
  exitSignal: GqlTradeSignal;
  ...
  unreinvestedDividends: number;
}

/** The signal that opened or closed a trade. */
export interface GqlTradeSignal {
  /** `SignalDirection` is `#[non_exhaustive]`, so a new library variant must not fail deserialization here. */
  direction: string;
  price: number;
  reason?: string;
  strength: number;
...
export interface Page_GqlEquityPoint {
  edges: Record<string, unknown>[];
  nodes: GqlEquityPoint[];
  pageInfo: Record<string, unknown>;
}
...
export interface Page_GqlTrade {
  edges: Record<string, unknown>[];
  nodes: GqlTrade[];
  pageInfo: Record<string, unknown>;
}
