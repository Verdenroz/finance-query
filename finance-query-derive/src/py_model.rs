...
/// If `ty` is `FormattedValue<X>` with X a known concrete type, return the
/// corresponding `::finance_query::PyFormattedValueX` path. Otherwise None.
fn formatted_value_wrapper(ty: &Type) -> Option<TokenStream2> {
    let inner = unwrap_generic(ty, "FormattedValue")?;
