...
!!! warning "Scanned filings are silently skipped"
    Only filings typed through fd.house.gov's e-filing system carry a text layer this adapter can read. Older or hand-signed PTRs are scanned images; those produce zero matching transactions rather than an error, since this adapter does not OCR them. Across a 240-filing sample spanning 2021-2026, 12.5% had no text layer and the rest extracted without error; the ignored `measure_scanned_filing_rate` test reproduces the figure.
