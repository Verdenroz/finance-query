...
class TestOscillators:
    async def test_get_rsi(self, historical_quotes, mock_finance_client, monkeypatch):
        ...
        assert round(result["Technical Analysis"]["2025-03-14"]["RSI"], 2) == 49.32
        assert round(result["Technical Analysis"]["2024-11-01"]["RSI"], 2) == 52.84
        assert round(result["Technical Analysis"]["2024-09-27"]["RSI"], 2) == 54.91
    ...
    async def test_get_cci(self, historical_quotes, mock_finance_client, monkeypatch):
        ...
        assert "Technical Analysis" in result
        assert round(result["Technical Analysis"]["2025-03-14"]["CCI"], 2) == -20.62
        assert round(result["Technical Analysis"]["2024-11-01"]["CCI"], 2) == -24.36
        assert round(result["Technical Analysis"]["2024-09-27"]["CCI"], 2) == 88.88
