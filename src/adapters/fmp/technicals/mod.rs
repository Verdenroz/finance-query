...
async fn fetch_daily_indicator(
    ...
    period: u32,
    type_: &str,
) -> Result<Vec<TechnicalIndicatorValueDTO>> {
...
/// Fetch daily MACD.
pub async fn daily_macd(symbol: &str, type_: &str) -> Result<Vec<TechnicalIndicatorValueDTO>> {
    let client = build_client()?;
    let path = format!(
        "/api/v3/technical_indicator/daily/{}",
        encode_path_segment(symbol)
...
mod tests {
    ...
    async fn test_daily_sma_mock() {
        ...
        let resp: Vec<TechnicalIndicatorValueDTO> = client
            .get(
                path,
