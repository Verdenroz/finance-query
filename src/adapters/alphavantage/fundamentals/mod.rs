...
pub async fn income_statement(symbol: &str) -> Result<FinancialStatementsDTO> {
    fetch_financial_statement(symbol, "INCOME_STATEMENT").await
}

/// Fetch balance sheet (annual and quarterly).
pub async fn balance_sheet(symbol: &str) -> Result<FinancialStatementsDTO> {
    fetch_financial_statement(symbol, "BALANCE_SHEET").await
}

/// Fetch cash flow statement (annual and quarterly).
pub async fn cash_flow(symbol: &str) -> Result<FinancialStatementsDTO> {
    fetch_financial_statement(symbol, "CASH_FLOW").await
}

async fn fetch_financial_statement(symbol: &str, function: &str) -> Result<FinancialStatementsDTO> {
    let client = build_client()?;
    let json = client.get(function, &[("symbol", symbol)]).await?;

    fn parse_reports(json: &serde_json::Value, key: &str) -> Vec<FinancialReportDTO> {
        json.get(key)
            .and_then(|v| v.as_array())
            .map(|arr| {
                arr.iter()
...
pub async fn earnings(symbol: &str) -> Result<EarningsHistoryDTO> {
    ...
    fn parse_earnings(json: &serde_json::Value, key: &str) -> Vec<EarningsDataDTO> {
        json.get(key)
            .and_then(|v| v.as_array())
            .map(|arr| {
                arr.iter()
                    .map(|e| EarningsDataDTO {
...
pub async fn dividends(symbol: &str) -> Result<Vec<DividendEventDTO>> {
    let client = build_client()?;
    let csv = client.get_csv("DIVIDENDS", &[("symbol", symbol)]).await?;
    parse_dividend_csv(&csv)
}

fn parse_dividend_csv(csv: &str) -> Result<Vec<DividendEventDTO>> {
...
pub async fn splits(symbol: &str) -> Result<Vec<SplitEventDTO>> {
    let client = build_client()?;
    let csv = client.get_csv("SPLITS", &[("symbol", symbol)]).await?;
    parse_split_csv(&csv)
}

fn parse_split_csv(csv: &str) -> Result<Vec<SplitEventDTO>> {
    let mut events = Vec::new();
    let mut lines = csv.lines();
...
pub async fn earnings_calendar() -> Result<Vec<EarningsCalendarEntryDTO>> {
    let client = build_client()?;
    ...
    for line in lines {
        let cols: Vec<&str> = line.split(',').collect();
        if !cols.is_empty() {
            entries.push(EarningsCalendarEntryDTO {
                symbol: cols[0].to_string(),
                name: cols.get(1).map(|s| s.to_string()),
...
pub async fn ipo_calendar() -> Result<Vec<IpoCalendarEntryDTO>> {
    let client = build_client()?;
    ...
    for line in lines {
        let cols: Vec<&str> = line.split(',').collect();
        if !cols.is_empty() {
            entries.push(IpoCalendarEntryDTO {
