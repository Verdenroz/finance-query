...
#[non_exhaustive]
pub struct MerchantAggregateDTO {
    /// Merchant name.
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[non_exhaustive]
pub struct MerchantHierarchyDTO {
    /// Merchant name.
    pub merchant: Option<String>,
    /// Parent company name.
    pub parent: Option<String>,
...
pub async fn merchant_aggregates(
    params: &[(&str, &str)],
) -> Result<PaginatedResponseDTO<MerchantAggregateDTO>> {
...
pub async fn merchant_hierarchy(
    params: &[(&str, &str)],
) -> Result<PaginatedResponseDTO<MerchantHierarchyDTO>> {
    let client = build_client()?;
    client
