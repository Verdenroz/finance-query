...
class DefeatBetaClient:
    ...
    async def get_earnings_transcript(
    ...
    ) -> dict[str, Union[str, list[dict[str, Union[str, int, datetime, list[str], dict[str, str]]]]]]:
        """
        ...
        try:
            ...
            if transcripts_df.empty:
                raise HTTPException(status_code=404, detail=f"No earnings transcripts found for symbol {symbol}")
            ...
            raise HTTPException(status_code=500, detail="defeatbeta-api package not properly installed") from e
    ...
    async def get_financial_statement(
        self, symbol: str, statement_type: str, frequency: str
    ) -> dict[str, Union[str, dict[str, dict[str, Union[str, int, float, None]]], dict[str, Union[str, int]]]]:
        """
        ...
        try:
            ...
            if not hasattr(ticker, method_name):
                raise HTTPException(status_code=400, detail=f"Method {method_name} not available for {symbol}")
            ...
            if financial_data is None:
                raise HTTPException(status_code=404, detail=f"No {statement_type} data found for symbol {symbol}")
            ...
            raise HTTPException(status_code=500, detail=f"Failed to fetch {statement_type}: {str(e)}") from e
    ...
    def _get_financial_method_name(self, statement_type: str, frequency: str) -> str:
        ...
        if statement_type == "income_statement":
            ...
            raise HTTPException(status_code=400, detail=f"Unsupported statement type: {statement_type}")

    def _format_financial_data(
        self, financial_data: Union[pd.DataFrame, object], symbol: str, statement_type: str, frequency: str
    ) -> dict[str, Union[str, dict[str, dict[str, Union[str, int, float, None]]], dict[str, Union[str, int]]]]:
        """
        ...
        try:
            ...
            if hasattr(financial_data, "get_data"):
                ...
                try:
                    ...
                    raise HTTPException(status_code=500, detail=f"Unable to convert financial data to DataFrame: {str(e)}") from e

            if df is None or df.empty:
                raise HTTPException(status_code=404, detail=f"No {statement_type} data found for {symbol}")
            ...
            formatted_data = {
                ...
                "frequency": frequency,
                "statement": data_dict,
                "metadata": {"source": "defeatbeta-api", "retrieved_at": datetime.now().isoformat(), "rows_count": len(df), "columns_count": len(df.columns)},
            }
            ...
            raise HTTPException(status_code=500, detail=f"Failed to format financial data: {str(e)}") from e
    ...
    def _format_transcript_data(
        self, transcripts_df: pd.DataFrame, symbol: str, quarter: Optional[str] = None, year: Optional[int] = None
    ) -> dict[str, Union[str, list[dict[str, Union[str, int, datetime, list[str], dict[str, str]]]]]]:
        """
        ...
        try:
            ...
            if not isinstance(transcripts_df, pd.DataFrame):
                raise HTTPException(status_code=500, detail="Invalid data format from defeatbeta-api")

            formatted_data = {"symbol": symbol.upper(), "transcripts": []}
            ...
            if not formatted_data["transcripts"]:
                raise HTTPException(status_code=404, detail=f"No transcripts found for {symbol} with the specified filters")
            ...
            raise HTTPException(status_code=500, detail=f"Failed to format transcript data: {str(e)}") from e

    def _process_transcript_row(
        self, row: pd.Series, quarter: Optional[str] = None, year: Optional[int] = None
    ) -> Optional[dict[str, Union[str, int, datetime, list[str], dict[str, str]]]]:
        """
        ...
        try:
            ...
            transcript = {
                ...
                "metadata": {"source": "defeatbeta-api", "retrieved_at": datetime.now().isoformat(), "transcripts_id": clean_value(row.get("transcripts_id"))},
            }
    ...
    def _filter_transcripts(
        self, transcripts: list[dict[str, Union[str, int, datetime, list[str], dict[str, str]]]], quarter: Optional[str] = None, year: Optional[int] = None
    ) -> list[dict[str, Union[str, int, datetime, list[str], dict[str, str]]]]:
        """
