...
@router.get(
    ...
    responses={
        ...
        500: {
            ...
            "content": {"application/json": {"example": {"detail": "Failed to fetch earnings transcript"}}}
        }
    }
)
async def get_earnings_transcript_endpoint(
    ...
    **Returns:**
    ...
    """
    return await get_earnings_transcript(
        symbol=symbol,
        quarter=quarter,
        year=year
    )
...
@router.get(
    ...
    dependencies=[Security(APIKeyHeader(name="x-api-key", auto_error=False))],
    responses={
        200: {"description": "Successfully retrieved latest earnings transcript"},
        404: {"description": "No transcripts found for symbol"}
    }
)
async def get_latest_earnings_transcript_endpoint(symbol: str):
    ...
    result = await get_earnings_transcript(
        symbol=symbol,
        quarter=None,
        year=None
    )
    ...
    if len(transcripts) > 1:
        ...
        transcripts.sort(
            key=lambda x: (
                x["year"],
                {"Q1": 1, "Q2": 2, "Q3": 3, "Q4": 4}.get(x["quarter"], 0)
            ),
            reverse=True
        )
