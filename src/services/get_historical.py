...
async def get_historical(symbol: str, time: TimePeriod, interval: Interval):
    stock = Ticker(symbol,asynchronous=True, retry=20, status_forcelist=[404, 429, 500, 502, 503, 504])
    ...
    if not isinstance(data.index.get_level_values('date'), pd.DatetimeIndex):
        data.index = data.index.set_levels(pd.to_datetime(data.index.get_level_values('date'), utc=True), level='date')
