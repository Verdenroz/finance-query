...
async def fetch_quotes(symbols: list[str], cookies: str, crumb: str) -> list[Quote]:
    ...
    all_quotes = await asyncio.gather(*(
        asyncio.gather(*(_get_quote_from_yahoo(symbol, cookies, crumb) for symbol in chunk))
        for chunk in chunks
    ))
...
async def _parse_yahoo_quote_data(summary_data: dict) -> Quote:
    ...
    pre_market_price = get_fmt(price_data, "preMarketPrice") if is_within_pre_market_time(
        price_data.get("preMarketTime", 0)) else None
    post_market_price = get_fmt(price_data, "postMarketPrice") if is_within_post_market_time(
        price_data.get("postMarketTime", 0)) else None
    ...
    quote_data = {
        ...
        "morningstar_rating": f"★{'★' * (stats.get('morningStarOverallRating', {}).get('raw', 0) - 1)}" if stats.get(
            'morningStarOverallRating') else None,
        "morningstar_risk_rating": get_morningstar_risk_rating(
            stats.get("morningStarRiskRating", {}).get('raw', -1)),
