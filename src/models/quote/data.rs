...
impl Quote {
    ...
    /// Build a Quote from a provider-agnostic [`crate::providers::types::QuoteData`].
    ///
    /// This is a partial conversion — many Yahoo-specific sub-module fields
    /// are left as `None`. When a full Quote is needed, use the Yahoo-specific
    /// [`from_response`](Quote::from_response) path instead.
    pub(crate) fn from_quote_data(data: crate::providers::types::QuoteData) -> Self {
        Self {
            symbol: data.symbol,
            ...
            from_currency: None,
            to_currency: None,
            regular_market_price: data.regular_market_price.map(FormattedValue::new),
            regular_market_change: data.regular_market_change.map(FormattedValue::new),
            regular_market_change_percent: data
                .regular_market_change_percent
                .map(FormattedValue::new),
            regular_market_time: None,
            regular_market_day_high: data.regular_market_day_high.map(FormattedValue::new),
            regular_market_day_low: data.regular_market_day_low.map(FormattedValue::new),
            regular_market_open: data.regular_market_open.map(FormattedValue::new),
            regular_market_previous_close: data
                .regular_market_previous_close
                .map(FormattedValue::new),
            regular_market_volume: data
                .regular_market_volume
                .map(|v| FormattedValue::new(v as i64)),
            market_state: None,
            day_high: None,
            day_low: None,
            open: None,
            ...
            average_daily_volume10_day: None,
            average_daily_volume3_month: None,
            average_volume: None,
            average_volume10days: None,
            volume: data
                .regular_market_volume
                .map(|v| FormattedValue::new(v as i64)),
            market_cap: data.market_cap.map(|v| FormattedValue::new(v as i64)),
            ...
            forward_pe: None,
            trailing_pe: data.pe_ratio.map(FormattedValue::new),
            beta: data.beta.map(FormattedValue::new),
            fifty_two_week_high: data.fifty_two_week_high.map(FormattedValue::new),
            fifty_two_week_low: data.fifty_two_week_low.map(FormattedValue::new),
            fifty_day_average: data.fifty_day_avg.map(FormattedValue::new),
            two_hundred_day_average: data.two_hundred_day_avg.map(FormattedValue::new),
            week_52_change: None,
            sand_p_52_week_change: None,
            dividend_rate: data.dividend_rate.map(FormattedValue::new),
            dividend_yield: data.dividend_yield.map(FormattedValue::new),
            ...
            current_ratio: None,
            quick_ratio: None,
            trailing_eps: data.eps_ttm.map(FormattedValue::new),
            forward_eps: None,
            book_value: None,
            sector: data.sector,
            sector_key: None,
            ...
            industry_key: None,
            industry_disp: None,
            long_business_summary: data.description,
            ...
            phone: None,
            full_time_employees: data.employees.map(|e| e as i64),
            category: None,
            fund_family: None,
            audit_risk: None,
            board_risk: None,
            ...
            overall_risk: None,
            time_zone_full_name: None,
            time_zone_short_name: None,
            gmt_off_set_milliseconds: None,
            ...
            // Yahoo-specific sub-modules — not available from provider-agnostic QuoteData
            earnings: None,
