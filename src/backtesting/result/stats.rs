...
/// Test fixtures shared across the `backtesting::result` test modules.
#[cfg(test)]
pub(super) mod fixtures {
    use chrono::NaiveDate;

    use super::super::{BacktestResult, EquityPoint, PerformanceMetrics};
    ...
    pub(crate) fn make_trade(pnl: f64, return_pct: f64, is_long: bool) -> Trade {
        Trade {
            side: if is_long {
                PositionSide::Long
            } else {
    ...
    pub(crate) fn ts(date: &str) -> i64 {
        let d = NaiveDate::parse_from_str(date, "%Y-%m-%d").unwrap();
        d.and_hms_opt(12, 0, 0).unwrap().and_utc().timestamp()
    }

    pub(crate) fn equity_point(timestamp: i64, equity: f64, drawdown_pct: f64) -> EquityPoint {
        EquityPoint {
            timestamp,
            equity,
            drawdown_pct,
...
#[cfg(test)]
mod tests {
    use super::fixtures::{equity_point, make_trade, ts};
    use super::*;
